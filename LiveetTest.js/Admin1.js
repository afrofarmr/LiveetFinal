...
describe('Admin', function () {
    ...
    it('Create Staff', function () {
        ...
        cy.get('#admin_fullname').type('Stella Venus')
        cy.get('#admin_username').type('astellavee')
        cy.get('#admin_email').type('astella@eleran.com')
        cy.get('#admin_password').type('stella07')
        cy.get(':nth-child(1) > :nth-child(2) > .custom-control-label').click()
        cy.get(':nth-child(6) > .custom-control-label').click()
        cy.get(':nth-child(2) > :nth-child(3) > .custom-control-label').click()
